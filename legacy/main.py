...
parser.add_argument(
    "--print-only", action="store_true", help="print list of available data"
)
parser.add_argument("--odir", default=".", help="directory to store tiles")
parser.add_argument(
    "--year", type=str, default="latest", help="specify year data captured"
)
parser.add_argument(
    "--all-years",
    action="store_true",
    help="download all available years between --year and latest",
)
parser.add_argument(
    "--open-browser", action="store_false", help="open browser i.e. do not run headless"
)
parser.add_argument(
    "--browser", type=str, default="chrome", help="choose between chrome and firefox"
)
...
parser.add_argument(
    "--point-cloud", "-pc", action="store_true", help="download point cloud"
)
...
args.required_products = [
    p
    for (p, b) in zip(products, [args.dsm, args.dtm, args.point_cloud, args.national])
    if b
]
if not any(args.required_products):
    raise Exception(
        "pick one or more products using the --point-cloud, --dsm or --dtm flags"
    )

if args.verbose and args.print_only:
    print("PRINT ONLY - no data will be downloaded")
...
if shp.area.values[0] > 561333677 or len(shp.explode(index_parts=True)) > 1:
    if args.verbose:
        "input geometry is large and or complex, tiling data."
    tile_input(shp, args)

if num_vertices(shp) > 1000:  # maximum number of vertics accepted by application
    if args.verbose:
        print("simplifying to <1000 vertices")
    simp = 10
    ...
    if args.verbose:
        print(
            "simplified polygon saved to:",
            os.path.join(args.tmp_d, args.tmp_n + ".shp"),
        )
...
with ZipFile(zipPath, "w") as zipObj:
    [
        ...
        for f in glob.glob(os.path.splitext(args.extent)[0] + "*")
        if not f.endswith(".zip")
    ]
    # [print(f) for f in glob.glob(os.path.splitext(args.extent)[0] + '*') if not f.endswith('.zip')]

if args.verbose:
    print("zip file saved to:", zipPath)

driver = download_tile(
    zipPath,
